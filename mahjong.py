...
def create_yamahai():
    tiles = [
        Tile(kind, str(value)) for kind in Tile.SUUPAI for value in range(1, 1 + 9)
    ]
    tiles += [Tile(Tile.JIHAI[0], value) for value, label in enumerate(Tile.WINDS, 1)]
    tiles += [Tile(Tile.JIHAI[1], value) for value, label in enumerate(Tile.COLORS, 1)]
    tiles *= 4
...
class Agari:
    ...
    def __repr__(self):
        return (
            f"[{repr(self.janto[0])},{repr(self.janto[1])}],"
...
class Titoitsu:
    ...
    def __repr__(self):
        return (
            f"[{repr(self.l_toitsu[0])},{repr(self.l_toitsu[0])}],"
...
def judge(tehai):
    ...
    for janto in l_janto:
        ...
        # 刻子の種類
        l_koutsu = sorted(
            [x for x in set(mentsu_kouho) if mentsu_kouho.count(x) >= 3],
            key=lambda hai: f"{hai.kind}{hai.value}",
        )
...
def agari_koutsu1(mentsu_kouho, janto, l_koutsu):
    ...
    for koutsu in l_koutsu:
        try:
            ...
            result.append(
                Agari([janto for x in range(2)], first, second, third, fourth)
            )
...
def agari_koutsu4(janto, l_koutsu):
    ...
    return [
        Agari(
            [janto for x in range(2)],
            Mentsu(Mentsu.KIND[1], [l_koutsu[0] for x in range(3)]),
...
def test():
    ...
    for pattern in [
        ...
        l_haipai.append([Tile("manzu", value) for value in list(pattern)])
    l_haipai.append(
        [
            Tile(Tile.SUUPAI[0], "1"),
            Tile(Tile.SUUPAI[0], "9"),
            Tile(Tile.SUUPAI[1], "1"),
            Tile(Tile.SUUPAI[1], "9"),
            Tile(Tile.SUUPAI[2], "1"),
            Tile(Tile.SUUPAI[2], "9"),
            Tile(Tile.JIHAI[0], Tile.WINDS[0]),
            Tile(Tile.JIHAI[0], Tile.WINDS[1]),
            Tile(Tile.JIHAI[0], Tile.WINDS[2]),
            Tile(Tile.JIHAI[0], Tile.WINDS[3]),
            Tile(Tile.JIHAI[1], Tile.COLORS[0]),
            Tile(Tile.JIHAI[1], Tile.COLORS[1]),
            Tile(Tile.JIHAI[1], Tile.COLORS[2]),
            Tile(Tile.SUUPAI[0], "1"),
        ]
    )
